...
//    }
    ...
    {
        $sliders = OperationSlider::orderBy('sort')->get();
...
//        });
        ...
        $operation_cats= OperationCat::where('status','active')->get();
        return view('front.index', compact('operation_cats','sliders', 'about', 'contact_home', 'blogs', 'services', 'gallery'), ['title' => 'index']);
    }
    ...
    public function service_list($lang='en')
    {
    ...
    public function blog_list($lang='en',$type = 'all')
    {
    ...
    public function blog_show($lang='en',$type, $id)
    {
    ...
    public function filter_car($lang='en',Request $request)
    {
        try {
            ...
            $cars = Estate::where('type', 'company')->where('status', 'آماده تحویل')->doesntHave('reserves')
            ...
            $cars_col = Estate::where('type', 'colleague')->where('status', 'آماده تحویل')->doesntHave('rents')
            ...
            $cars_id = Estate::where('type', 'company')->where('status', 'آماده تحویل')->doesntHave('reserves')
                ->orWhere('type', 'company')->where('status', 'آماده تحویل')->whereHas('reserves', function ($r) use ($to_date, $from_date) {
                    $r->where('status', 'فعال')->whereDate('reserve_start_date', '>', $to_date)->whereDate('reserve_end_date', '<', $from_date);
                })->orWhere('type', 'company')->where('status', 'در حال اجاره')->whereDate('return_date', '<', $from_date)->select('id')->get()->toArray();

            $cars_col_id = Estate::where('type', 'colleague')->where('status', 'آماده تحویل')->doesntHave('rents')
                ->orWhere('type', 'colleague')->where('status', 'آماده تحویل')->whereHas('rents', function ($r) use ($to_date, $from_date) {
                    $r->where('status', 'active')->whereDate('from_date', '>', $to_date)->whereDate('to_date', '<', $from_date);
                })->orWhere('type', 'colleague')->where('status', 'در حال اجاره')->whereDate('return_date', '<', $from_date)->select('id')->get()->toArray();

            $estates2 = Estate::where('type', 'company')->whereIN('status', ['آماده تحویل', 'در حال اجاره'])->whereNotIN('id', $estates_id)->get();


            $estates2_col = Car::where('type', 'colleague')->whereIN('status', ['آماده تحویل', 'در حال اجاره'])->whereNotIN('id', $estates_col_id)->get();

            //  $estates2=$estates2->merge($estates2_col);

            $whatsapp_estate = Contact::first()->whatsapp_estate;
            ...
            return view('front.estate.index', compact('estates', 'estates2', 'whatsapp_estate', 'from_date', 'to_date', 'place'), ['title' => read_lang_word('هدر-صفحات-داخلی', 'estate_list')]);
    ...
    public function reserve_estate($lang='en',$estate_id,$set=null,Request $request)
    {
        try {
            $sett = Setting::first();
            ...
            $estate = Car::findOrFail($estate_id);
            $from_date = $request->from_date;
            $to_date = $request->to_date;
            $place = blank($request->location) ? 'no_select' : $request->location;
            $reserve_crm=null;
            ...
            if ($estate->type == 'company') {
                $item = Car::where('type', 'company')->where('id', $estate_id)->where('status', 'آماده تحویل')->doesntHave('reserves')
                    ->orWhere('type', 'company')->where('id', $estate_id)->where('status', 'آماده تحویل')->whereHas('reserves', function ($r) use ($to_date, $from_date) {
                        $r->where('status', 'فعال')->whereDate('reserve_start_date', '>', $to_date)->whereDate('reserve_end_date', '<', $from_date);
                    })->orWhere('type', 'company')->where('id', $estate_id)->where('status', 'در حال اجاره')->whereDate('return_date', '<', $from_date)->first();
            }
            elseif ($estate->type == 'colleague') {
                $reserve_crm = 'yes';

                $item = Car::where('type', 'colleague')->where('id', $estate_id)->where('status', 'آماده تحویل')->doesntHave('rents')
                    ->orWhere('type', 'colleague')->where('id', $estate_id)->where('status', 'آماده تحویل')->whereHas('rents', function ($r) use ($to_date, $from_date) {
                        $r->where('status', 'active')->whereDate('from_date', '>', $to_date)->whereDate('to_date', '<', $from_date);
                    })->orWhere('type', 'colleague')->where('id', $estate_id)->where('status', 'در حال اجاره')->whereDate('return_date', '<', $from_date)->first();
            }
            if(!$item)
            {
                if(blank($set))
                {
                    if (session()->has('request_phone')) {
                        if ($sett && !blank($sett->email)) {
                            ...
                            if($estate->status!='در حال اجاره' && $car->status!='آماده تحویل')
                            {
                                $text .= '<strong>';
                                $text .= ' رزرو قبلی ';
                                $text .= '</strong>';
    ...
    public function rent_car_level1($lang='en',$car_id, $from_date, $to_date, $place, $reserve_crm = null, Request $request)
    {
...
//            }
    ...
    public function rent_car_level1_show($lang='en',$car_id, Request $request)
    {
    ...
    public function car_post($lang='en',$id, Request $request)
    {
    ...
    public function rent_car_level1_post($lang='en',$car_id, $from_date, $to_date, $place, $reserve_crm = 'no', Request $request)
    {
...
//option add
    ...
    public function information_get($lang='en',$id)
    {
    ...
    public function operation_show($lang='en'){

        return view('front.operation.show');
