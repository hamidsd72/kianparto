...
class ProductController extends Controller {
    ...
    public function show( $lang, $id, $slug ) {
        ...
        $item   = OperationTopContent::where('status', 'active')->findOrFail($id);
        $links  = OperationCat::where('parent_id', 0)->where('status', 'active')->get();
        return view('front.product.product', compact('slug', 'item','links'), ['title' => read_lang_word('هدر-صفحات-داخلی', 'about')]);
    }
