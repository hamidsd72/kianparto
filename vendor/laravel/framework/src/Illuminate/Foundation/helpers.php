...
if (!function_exists('optimize_all_pic')) {
    function optimize_all_pic()
    {
      ...
      $photos=EstatePhoto::where('opt','no')->get();
     	foreach($photos as $ph)
        {
...
if (!function_exists('crm_token_delete')) {
    function crm_token_delete()
    {
        ...
        $item=EstateToken::where('updated_at','<',$end_date)->first();
...
if (!function_exists('nerkh_set')) {
    function nerkh_set($estate_id,$from_date,$to_date,$place=null)
    {
        $setting=Setting::first();
        $add_price_delivery=0;
        if($setting && $place=='در محل مشتری')
        {
            $add_price_delivery=$setting->delivery_estate;
        }
        $sub_percent_bimeh=0;
        $add_percent_col=0;
        $estate=Estate::find($estate_id);
        if($estate && $estate->type=='colleague')
        {
            $work=UserWork::where('user_id',$estate->colleague_id)->first();
            if($work)
            {
       ...
       $price=EstateNerkh::where('estate_system_id',$estate_id)->orderByDesc('id')->first();
...
if (!function_exists('nerkh_set_day')) {
    function nerkh_set_day($estate_id)
    {
        $setting=Setting::first();
        $add_percent_col=0;
        $percent_d=0;
    ...
    function login_crm()
    {
        TblNotError::create([
            ...
            if ($response && $response["success"]) {
                ...
                {
                    $estate_t=EstateToken::first();
                    $estate_t->token=$result1["sessionName"];
                    $estate_t->update();
                }
...
if (!function_exists('export_estate_list')) {
    function export_estate_list($start,$count)
    {
        TblNotError::create([
...
if (!function_exists('pic_set_estate')) {
    function pic_set_estate($data,$check=null)
    {
        $address_pic_set=false;
        $address_pic=url('assets/front/img/home1/estate-8.jpg');
        $estate=Estate::find($data->id);
      
        if($estate)
        {
            if($estate->estate_photo && is_file($estate->estate_photo->path))
            {
                $address_pic=url($estate->estate_photo->path);
                $address_pic_set=true;
            }
            elseif($estate->estate_photos && count($estate->estate_photos) && is_file($estate->estate_photos[0]->path))
            {
                $address_pic=url($estate->estate_photos[0]->path);
                $address_pic_set=true;
            }
             elseif($estate->photo)
            {
                $address_pic=url($estate->photo->path);
                $address_pic_set=true;
            }
            elseif($estate->photos && count($estate->photos))
            {
                $address_pic=url($estate->photos[0]->path);
                $address_pic_set=true;
            }
            else
            {
                $estate_pic=EstatePic::where('brand',str_replace(' ','_',$data->brand))->where('model',str_replace(' ','_',$data->model))->where('year',$data->year)->where('motor',$data->motor_power)->where('color',$data->color)->first();
                if($estate_pic && $estate_pic->photo && is_file($estate_pic->photo->path))
                {
                    $address_pic=url($estate_pic->photo->path);
                    $address_pic_set=true;
                }
            }
        }
...
if (!function_exists('getStoriesFromType')) {
    function getStoriesFromType($item,$type)
    {
        return $item->stories()->where('type', $type)->orderByDesc('id')->get();
